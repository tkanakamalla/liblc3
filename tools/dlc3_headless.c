...
static struct parameters parse_args(int argc, char *argv[])
{
    static const char *usage =
        ...
        "\t-i\t"     "Input samplerate\n"
        "\t-u\t"     "Input frame duration\n"
        "\t-c\t"     "Input channels\n"
        "\t-s\t"     "Input samples\n"
        "\t-f\t"     "Frame bytes (byte count)"
        "\n";
    ...
    for (int iarg = 1; iarg < argc; ) {
        ...
        if (arg[0] == '-') {
            ...
            switch (opt) {
                case 'b': case 'r': case 'i': case 'u': case 'c': case 's': case 'f':
                    if (iarg >= argc)
            ...
            switch (opt) {
                ...
                case 'i': p.in_srate_hz = atoi(optarg); break;
...
int main(int argc, char *argv[])
{
    ...
    int frame_us, srate_hz, nch, nsamples, in_frame_bytes;

    // if (lc3bin_read_header(fp_in, &frame_us, &srate_hz, &nch, &nsamples) < 0)
    //     error(EINVAL, "LC3 binary input file");
        frame_us = p.frame_us;
        srate_hz = p.in_srate_hz;
        nch = p.channels;
        nsamples = p.enc_samples;
        in_frame_bytes = p.frame_bytes;
    ...
    for (int i = 0; i * frame_samples < encode_samples; i++) {
        ...
        int frame_bytes = lc3bin_read_data_only(fp_in, nch, in, in_frame_bytes);
